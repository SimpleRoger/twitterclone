...
export default function Tweet({ data, id }) {
  ...
  //listens for changes in document
  useEffect(() => {
    if (!id) return;
    //runs each time there is a change to collection
    const unsubscribe = onSnapshot(doc(db, "posts", id), (doc1) => {
      setLikes(doc1.data()?.likes);
      setComments(doc1.data()?.comments);
    });
    return unsubscribe;
  }, []);
  return (
      ...
      <div className="p-3 ml-16 text-gray-500 flex space-x-14">
        ...
        >
          {likes.includes(user.uid) ? (
            ...
            <HeartIcon className="w-5 cursor-pointer hover:pink-green-500" />
          )}{" "}
        ...
        <UploadIcon className="w-5 ccursor-not-allowed" />
      </div>
    </div>
  );
}
